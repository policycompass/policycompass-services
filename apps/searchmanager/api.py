...
class Base(APIView):
     ...
     def get(self, request, format=None):
        result = {
            ...
            "Rebuild Index Service only for indicators": reverse('rebuildindex_indicator', request=request),
             "Rebuild Index Service only for fuzzy maps": reverse('rebuildindex_fuzzymap', request=request),
            "Create or Update Index for an itemtype ('metric','visualization','event','dataset','fuzzymap', 'indicator')": reverse('update_index_item', request=request,kwargs={'itemtype':'metric','itemid':26}),
            "Delete Index for an itemtype ('metric','visualization','event','dataset','fuzzymap', 'indicator')": reverse('delete_index_item', request=request,kwargs={'itemtype':'metric','itemid':26})
        }
