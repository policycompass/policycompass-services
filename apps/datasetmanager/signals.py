...
def update_document_on_search_service(sender, **kwargs):
     ...
     if (kwargs.get('created', True) and not kwargs.get('raw', False)):
         ...
         indexDocumentThread(curDataset.id, 'dataset').start()
...
def delete_document_on_search_service(sender, **kwargs):
     ...
     api_url = settings.PC_SERVICES['references']['base_url'] + settings.PC_SERVICES['references']['deleteindexitem'] + '/dataset/' + str(curDataset.id)
...
class indexDocumentThread(threading.Thread):
    ...
    def run(self):
        ...
        api_url = settings.PC_SERVICES['references']['base_url'] + settings.PC_SERVICES['references']['updateindexitem'] + '/dataset/' + str(self.itemid)
        #Execute the API call
        response = requests.post(api_url)
        #Print the response of the API call to console
        print(response.text)
